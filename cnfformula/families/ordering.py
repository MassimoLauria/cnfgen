...
@cnfformula.families.register_cnf_generator
def DenseOrderingPrinciple(size,degree=None,seed=None,total=True):
    ...
    Arguments:
    - `size` : size of the domain
    - `degree`: size of the neighbour sets in the sparse version
    ...
    if degree is None:
        ...
        if total:
            name = "Sparse dense linear ordering principle on complete graph with {0} nodes with neighbour sets of size {1}".format(size, degree)
        else:
            name = "Sparse dense ordering principle on complete graph with {0} nodes with neighbour sets of size {1}".format(size, degree)
...
class DOPCmdHelper(object):
    ...
    def build_cnf(args):
        ...
        - `args`: command line options
        """
        return DenseOrderingPrinciple(args.N,total=args.total)

@cnfformula.cmdline.register_cnfgen_subcommand
class SparseDOPCmdHelper(object):
    """Command line helper for Dense ordering principle formulas
    """
    name='dopsparse'
    description='dense ordering principle (sparse version)'
    ...
    def setup_command_line(parser):
        ...
        """
        parser.add_argument('N',metavar='<N>',type=int,help="domain size")
        parser.add_argument('degree',metavar='<degree>',type=int,help="number of neighbours per vertex for the dense axioms")
        g=parser.add_mutually_exclusive_group()
        g.add_argument('--total','-t',default=True,action='store_true',help="assume a total order")
        g.add_argument('--no-total',default=False,action='store_false',dest='total',help="don't assume a total order")
    ...
    def build_cnf(args):
        ...
        - `args`: command line options
        """
        return DenseOrderingPrinciple(args.N,degree=args.degree,total=args.total)
