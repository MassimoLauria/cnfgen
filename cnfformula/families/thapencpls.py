...
def ThapenCPLSFormula(a, b, c):
    ...
    def bin_ineq(f_i_x, x_prime, bits): return [(not bool(x_prime & (1<<j)), f_i_x[j]) for j in xrange(bits)]
...
class ThapenCPLSCmdHelper(object):
    ...
    def setup_command_line(parser):
        ...
        parser.add_argument('b',metavar='<b>',type=int,help="Number of nodes per level in the graph (must be a power of two)")
        parser.add_argument('c',metavar='<c>',type=int,help="Number of colours (must be a power of two)")
